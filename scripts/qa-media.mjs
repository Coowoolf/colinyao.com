...
await pg.waitForFunction(() => window.deck && window.deck.slides && window.deck.slides.length === 66);
...
// ── 3) 媒体行为 · P25 视频页（从 P24 自然走入 → 播 → 翻页） ──
await pg.evaluate(() => window.deck.go(23));
await pg.waitForTimeout(300);
// 推满 P24 的 step 后再前进
await pg.evaluate(() => {
  const d = window.deck, s = d.slides[23];
  const mx = Math.max(0, ...[...s.querySelectorAll("[data-step]")].map((e) => +e.dataset.step));
  for (let st = 1; st <= mx; st++) s.querySelectorAll(`[data-step="${st}"]`).forEach((e) => e.classList.add("on"));
  if (d.step !== undefined) d.step = mx;
});
await pg.keyboard.press("ArrowRight"); // P24 → P25（进入视频页，不播）
await pg.waitForTimeout(400);
const v0 = await pg.evaluate(() => ({ i: window.deck.i }));
await pg.keyboard.press("ArrowRight"); // 第一按：播视频
await pg.waitForTimeout(900);
const v1 = await pg.evaluate(() => {
  const d = window.deck, x = d.slides[24].querySelector("video[data-dm]");
  return { i: d.i, playing: !!x && !x.paused, ind: d.slides[24].classList.contains("dm-playing"), err: x && x.error ? x.error.code : 0, canH264: document.createElement('video').canPlayType('video/mp4; codecs="avc1.42E01E"') };
});
...
// ── 4) prev 复位 + P 键 ───────────────────────────────
await pg.evaluate(() => window.deck.go(24));
