...
const HERO = { 1: 'three-engines' };     // hero-art 只上封面
const fails = [];
...
const sw = await pg.evaluate(() => {
  ...
  return {
    ...
    lt: getComputedStyle(s.querySelector('.hero-art.lt')).display,
    dk: getComputedStyle(s.querySelector('.hero-art.dk')).display,
  };
});
