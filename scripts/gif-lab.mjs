// 动效实录 · LAB 页专用（gif-info-lab 的 DECK 可配版 —— 母本把 deck 路径写死了，
//   旗舰录不了；这一支只多一个 DECK env，其余逐行同源）：**TOUR.pace 定拍**（容器里是软渲染，按真实 dt 录出来会快 3–4×）。
//   钟只在 TOUR.step() 里按 1/fps 前进 ⇒ 录多久就是多久，GIF 的速度可以当证据看。
//   DOM 动效在录制期一律掐掉（3D 页上会动的是 canvas；poster 已经淡出，
//   留着 DOM 动画只会与定拍的 3D 打架）。
// 用法：DECK=/decks/convoai-lab.html PAGE=2 SECS=6 FPS=12 THEME=dark STEPS=1 \
//       CLIP=100,760,1720,240 OUT=/tmp/gif-lab-p2 node scripts/gif-lab.mjs
...
const OUT = process.env.OUT || `/tmp/gif-lab-p${PAGE}`;
...
await pg.goto(`${BASE}${DECK}?lab=hold#${PAGE}`, { waitUntil: 'load' });
