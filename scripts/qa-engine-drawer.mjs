...
// 断言：iframe 内 .pp 数 == 13；iframe 内 html[data-theme] 与宿主一致（浅浅 / 深深各一组）
...
for (const theme of ['light', 'dark']) {
  ...
  const r = await pg.waitForFunction(() => {
    ...
    return {
      ...
      title: d.title,
      sig13: d.querySelector('.slide[data-p="13"] .sig')?.textContent,
    };
  ...
  if (r.pp !== 13) fails.push(`[${theme}] iframe .pp 数 ${r.pp} != 13`);
  if (r.sec !== 13) fails.push(`[${theme}] iframe section 数 ${r.sec} != 13`);
  ...
  if (r.sig13 !== '13/13') fails.push(`[${theme}] iframe P13 sig「${r.sig13}」!= 13/13`);
  console.log(`· ${theme}：.pp=${r.pp} section=${r.sec} 宿主=${r.host || 'light'} iframe=${r.inner || 'light'} sig13=${r.sig13}`);
  await ctx.close();
}
await b.close();
console.log(fails.length ? '✗ FAIL\n' + fails.map(f => '  ' + f).join('\n') : '✓ PASS · 抽屉两组主题均跟随宿主 · iframe 13 页');
